...
def extract_file_set_two():
    ...
    extract_zip('main.zip', script_dir)
    extract_zip('dlc_models_raw.7z', script_dir)
    extract_zip('waw_textures_raw.7z', script_dir)
...
def print_complete_message():
    with open(f'{temp_dir}/End_Message.txt', 'r') as f:
        print(f.read())
    print('Credits to Shipudden for Map Maker')
    print('Credits to arrowsv for the World at War Texture set, Stock Model Set, and DLC Model Set')
